...
describe("ai config adapters", () => {
  ...
  test("save persists and reloads, hiding the key behind a hint", async () => {
    const db = memoryDb()
    const save = makeSaveConfig(db, () => "2026-01-01T00:00:00.000Z")
    ...
    const internal = await makeLoadStoredConfig(db)()
  ...
  test("apiKey empty string clears the key", async () => {
    const db = memoryDb()
    const save = makeSaveConfig(db)
    await save({ style: { apiKey: "sk-a" } })
    const after = await save({ style: { apiKey: "" } })
  ...
  test("corrupted stored JSON falls back to defaults", async () => {
    ...
    const config = await makeLoadConfig(db)()
    expect(toWireConfig(defaultStoredConfig())).toEqual(config)
  })
})
